...
async def main():
    ...
    candidate = await extract_candidate_profile(sample_resume)

    print("\nCandidate profile extracted.\n")

    # 2. Read the real job description
    with open("../jpmorgan_jd.txt", "r", encoding="utf-8") as file:
        job_description = file.read()

    # 3. Extract structured job information
    job = await extract_job_profile(job_description)

    print("Job profile extracted.\n")

    # 4. Match candidate against job
